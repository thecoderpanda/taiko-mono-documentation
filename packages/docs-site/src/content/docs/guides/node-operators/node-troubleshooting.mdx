...
### Block batch iterator callback error="failed to fetch tx list: sidecar not found"

You may be using an outdated version of `simple-taiko-node`, please pull the most recent version try again.

If problems still persist, check that your beacon client is functioning normally and fully synced:
also ensure that `L1_BEACON_HTTP` and `L1_ENDPOINT_WS` are set to valid endpoints that are fully synced!
